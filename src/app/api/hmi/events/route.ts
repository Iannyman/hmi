...
import { Device } from "@/lib/domain/device";
import { Line } from "@/lib/domain/line";
 ...
 */
function serializeDevice(device: Device, stationId: string) {
...
export async function GET(request: NextRequest) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const onStationUpdated = (station: Station) => {
        ...
        sendEvent("station:updated", {
          ...
          devices: stationDevices.map((d: Device) => serializeDevice(d, station.id)),
        });
      };

      // Subscribe to Device updates (when devices are subscribed on a specific station)
      const onDeviceUpdated = ({ stationId, device }: { stationId: string; device: Device }) => {
