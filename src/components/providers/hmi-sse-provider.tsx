...
import { useHMIDataContext, LineData } from "./hmi-data-provider";
    ...
    const connect = () => {
      ...
      eventSource.addEventListener("device:updated", (e: MessageEvent) => {
        ...
        if (station && station.devices) {
          const deviceIndex = station.devices.findIndex((d: { id: string }) => d.id === device.id);
          if (deviceIndex !== -1) {
      ...
      if (currentData.line || currentData.stations.size > 0) {
        setHmiData({
          line: currentData.line ?? {
            ...
            efficiency: 0,
            scrapRate: 0,
            order: { type: "", quantity: 0, contract: "" },
          },
