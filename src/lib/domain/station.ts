...
export class Station extends EventEmitter implements DomainObject {
  ...
  constructor(id: string, nodeMapper: NodeMapper, opcuaService: OPCUAService) {
    super();
    this.id = id;
  ...
  async subscribe(callback: (station: Station) => void, includeDevices: boolean = false): Promise<void> {
    ...
    if (!this.nodeIds) {
      ...
      for (const path of paths) {
        try {
          this.nodeMapper.getNodeId(path);
  ...
  private handleSubscriptionChanges(
    ...
    const _varName = this.nodeIdToName.get(nodeId) || "unknown";
    void _varName;
  ...
  private mapStatus(status: string): StationStatus {
    ...
    if (statusLower.includes("disabled")) return "disabled";
    return "setup";
  }
