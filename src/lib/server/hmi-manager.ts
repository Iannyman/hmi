...
export class HMIManager extends EventEmitter {
   ...
   */
  async initialize(browseTree: BrowseTreeNode[]): Promise<void> {
   ...
   */
  async rediscoverStructure(browseTree: BrowseTreeNode[]): Promise<StructureChange[]> {
    console.log("Re-discovering HMI structure...");

    const { changes } = this.nodeMapper.updateNodeIdMap(browseTree);
   ...
   */
  onDeviceAdded(callback: (event: { stationId: string; device: Device }) => void): void {
   ...
   */
  onDeviceRemoved(callback: (event: { stationId: string; device: Device }) => void): void {
   ...
   */
  onDeviceUpdated(callback: (event: { stationId: string; device: Device }) => void): void {
   ...
   */
  offDeviceAdded(callback: (event: { stationId: string; device: Device }) => void): void {
   ...
   */
  offDeviceRemoved(callback: (event: { stationId: string; device: Device }) => void): void {
   ...
   */
  offDeviceUpdated(callback: (event: { stationId: string; device: Device }) => void): void {
  ...
  private async subscribeToDeviceErrorMessages(): Promise<void> {
    ...
    await this.opcuaService.subscribe(
      errorNodeIds,
