...
import { useOPCUA } from "@/hooks/use-opcua";
import { useHMIManager } from "@/hooks/use-hmi-manager";
import { useConnectionSetter } from "@/hooks/use-connection";
    ...
    const autoBrowseAndInitialize = async () => {
      if (isConnected && !hmiInitialized && !hmiInitializing && !hasStartedReconnectingRef.current) {
        ...
        try {
          ...
          const serverInterfaces = (treeData as any[]).find(
            (node: any) => node.displayName === "ServerInterfaces"
          );
          ...
          const hmi = serverInterfaces?.children?.find(
            (child: any) => child.displayName === "hmi"
          );
